...
public class ConstrainedTaskScheduler : TaskScheduler
{
    ...
    private readonly ConcurrentHashSet<Task> _scheduledTasks = [];
    ...
    {
        Console.WriteLine($"TryExecuteTaskInline: {taskWasPreviouslyQueued}");
    ...
    private async void WorkerLoop(object? state)
    {
        ...
        try
        {
            ...
            {
                if (TryExecuteTask(task) && task.IsCompletedSuccessfully)
                {
                    while (task is Task<Task> taskOfTask && !taskOfTask.Result.IsCompleted)
                    {
                        task = taskOfTask.Result;
