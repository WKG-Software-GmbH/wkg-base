...
public class ConcurrentHashSet<T> : IReadOnlyCollection<T>, ICollection<T>
{
    ...
    private ConcurrentHashSet(int concurrencyLevel, int capacity, bool growLockArray, IEqualityComparer<T>? comparer)
    {
        ...
        ConcurrentHashSet<T>.Node[] buckets = new Node[capacity];
    ...
    public bool TryGetValue(T equalValue, [MaybeNullWhen(false)] out T actualValue)
    {
        ...
        ConcurrentHashSet<T>.Node? current = Volatile.Read(ref tables.Buckets[bucketNo]);
    ...
    private void GrowTable(Tables tables)
    {
        ...
        try
        {
            ...
            ConcurrentHashSet<T>.Node[] newBuckets = new Node[newLength];
            int[] newCountPerLock = new int[newLocks.Length];

            // Copy all data into a new table, creating new nodes for all elements
            for (int i = 0; i < tables.Buckets.Length; i++)
            {
                ConcurrentHashSet<T>.Node? current = tables.Buckets[i];
                while (current != null)
                {
                    ConcurrentHashSet<T>.Node? next = current.Next;
    ...
    {
        ConcurrentHashSet<T>.Node?[] buckets = _tables.Buckets;
        for (int i = 0; i < buckets.Length; i++)
        {
            for (ConcurrentHashSet<T>.Node? current = buckets[i]; current != null; current = current.Next)
            {
