    ...
    where THandle : unmanaged
{
    ...
    private ConcurrentBitmap56State _dataMapState;
    ...
    public PrioFastBitmap56Qdisc(THandle handle, Predicate<object?>? predicate, IClasslessQdiscBuilder localQueueBuilder, IClassifyingQdisc<THandle>[] children, int maxConcurrency) : base(handle, predicate)
    {
    ...
    private bool IsEmptyInternal => ConcurrentBitmap56.VolatileRead(ref _dataMapState).IsEmpty(_children.Length);
    ...
    protected override bool TryDequeueInternal(int workerId, bool backTrack, [NotNullWhen(true)] out AbstractWorkloadBase? workload)
    {
        ...
        while (!IsEmptyInternal)
        {
            for (int index = 0; index < children.Length; index++)
            {
                ...
                ConcurrentBitmap56 dataMapSnapshot = ConcurrentBitmap56.VolatileRead(ref _dataMapState);
                if (!dataMapSnapshot.IsBitSet(index))
                {
                    // skip empty children
                    continue;
                }
                // usually we should succeed first try, so we use the existing token from our read earlier
                byte token = dataMapSnapshot.GetToken();
                ...
                do
                {
                    if (expiredTokens != 0)
                    {
                        ...
                        token = ConcurrentBitmap56.VolatileRead(ref _dataMapState).GetToken();
                    }
                    ...
                    expiredTokens++;
                } while (!ConcurrentBitmap56.TryUpdateBitUnsafe(ref _dataMapState, token, index, isSet: false));
    ...
    protected override bool TryPeekUnsafe(int workerId, [NotNullWhen(true)] out AbstractWorkloadBase? workload)
    {
        while (!IsEmptyInternal)
        {
            ...
            for (int i = 0; i < children.Length; i++)
            {
                ...
                if (ConcurrentBitmap56.VolatileRead(ref _dataMapState).IsBitSet(i) && children[i].TryPeekUnsafe(workerId, out workload))
                {
                    return true;
                }
            }
    ...
    protected override void OnWorkScheduled()
    {
        ...
        ConcurrentBitmap56.UpdateBitUnsafe(ref _dataMapState, index, isSet: true);
